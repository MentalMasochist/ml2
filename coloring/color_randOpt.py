...
def get_colorvec_neighbor(colorvec):
	""" increase or decrease the color by one
	"""
	new_colorvec = copy.deepcopy(colorvec)            # copy vector
	bit_idx = np.random.randint(len(new_colorvec))  # get a random bit
	if (new_colorvec[bit_idx] == 0):                # flip the bit 
		new_colorvec[bit_idx] += 1
	else:
		if random() < 0.5:
			new_colorvec[bit_idx] += 1
		else:
...
def color_RandomizedHillClimb(colorvec_size, fit_func, iterations, restarts, verbose=False, ofilename='./results/rhc_progress.csv'):
	...
	for r in xrange(restarts):
		...
		colorvec = np.random.randint(colorvec_size,size=colorvec_size)

		# start with initial values
		old_fit = fit_func(colorvec)	
		old_vec = colorvec
		...
		for i in xrange(iterations):
			# take a random walk
			new_colorvec = get_colorvec_neighbor(colorvec)
			# get current fit
			new_fit = fit_func(new_colorvec)
			# assign a new fit function
			if new_fit >= max_fit:
				max_fit = new_fit
				max_vec = new_colorvec
				colorvec = copy.deepcopy(new_colorvec)
				if verbose:
...
def color_SimulatedAnnealing(colorvec_size, fit_func, iterations, restarts, T=1.0, alpha=0.999, verbose=False, ofilename='./results/sa_progress.csv'):
	...
	for r in xrange(restarts):
		...
		for i in xrange(iterations):
			# sample a neighbor
			new_vec = get_colorvec_neighbor(colorvec) 		
			# get current fit
...
def color_GeneticAlgorithm(colorvec_size, fit_func, population_size=100, generations=10000, restarts=1, child_per_parent=1, mutation_prob=0.1, crossover_split=0.5, verbose=False, ofilename='./results/ga_progress.csv'):
	""" Genetic Algorithm
		parameters:
			- population size
	...
	for r in xrange(restarts):
		...
		population = np.random.randint(colorvec_size,size=(population_size,colorvec_size))		
		# for each generation
...
def generate_offspring(population, mutation_prob, crossover_split):
	...
	if mutation_prob > random():
		offspring = list(get_colorvec_neighbor(np.array(offspring)))
	return offspring

def color_MIMIC(colorvec_size, fit_func, iterations=10, restarts=1, init_population_size=100, n_samples=100, theta_percentile=0.8, verbose=False, ofilename='./results/mimic_progress.csv'):
	""" MIMIC algorithm
	...
	for r in xrange(restarts):
		...
		population = np.random.randint(colorvec_size,size=(init_population_size,colorvec_size))		
		# get the fitness for each function
		fitness = np.apply_along_axis(fit_func, 1, population)
